              ...
              const TestData *data1, const TestData *data2, int *err)
{
  ...
  if (data1->nCols != data2->nRows) {
    *err = EDOM;
    ...
    TRACE("multiplying %p x %p to %p", multiplicand, multiplier, product);
...
{
  fprintf(stderr,
          "usage %s [<options>] <host> <port> [<filename>...]\n"
          "  <host>       the host name or ip address for the previously\n"
          "               started server\n"
          "  <port>       the port number for the previously started server\n"
...
getOptsPass1(struct option options[], int argc, const char *argv[], Opts *optsP)
{
  ...
  optsP->isErr = optsP->isErr || argc < optind + 2;
  if (!optsP->isErr) {
    optsP->hostName = argv[optind];
    optsP->portNo= argv[optind + 1];
  }
}
...
getOptsPass2(struct option options[], int argc, const char *argv[], Opts *optsP)
{
  ...
  if (!(optsP->isErr = optsP->isErr || (optind >= argc + 1))) {
    //skip hostname and portno which were read in getOptsPass1()
...
main(int argc, const char *argv[])
{
  ...
  MatrixMul *matMul = newMatrixMul(opts.hostName, opts.portNo, trace, &err);
